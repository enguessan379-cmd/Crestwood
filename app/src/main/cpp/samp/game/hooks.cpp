...
void NvUtilInit_hook() {
    Log("NvUtilInit");
    LOGI("Storage located at %s", g_pszStorage ? g_pszStorage : "(null)");
    NvUtilInit();
    ApplyFPSPatch(120);
    ReadSettingFile();
...
#else
    CHook::InlineHook(g_libGTASA + 0x5D6EC4, &CFont_PrintString_hook, &CFont_PrintString);
#endif
