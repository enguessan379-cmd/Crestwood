...
public final class GameThread {
    ...
    private final Object nativeStartupLock = new Object();
    private boolean nativeDataSetupDelivered;
    private boolean nativeActivityCreatedDelivered;
    private GameActivityBase pendingStartupActivity;
    private GameView pendingStartupView;
    private boolean pendingStartupFirstInit;
    ...
    public void setFixedSurfaceSize(int width, int height) {
        ...
        if (this.cachedSurfaceHolder != null && width > 0 && height > 0) {
            this.cachedSurfaceHolder.setFixedSize(width, height);
        }
    }

    public void clearFixedSurfaceSize() {
        this.fixedWidth = 0;
        this.fixedHeight = 0;
    }
    ...
    public void onActivityCreated(final GameActivityBase activity, final GameView view, final boolean firstInit) {
        ...
        synchronized (nativeStartupLock) {
            pendingStartupActivity = activity;
            pendingStartupView = view;
            pendingStartupFirstInit = firstInit;
        }
    ...
    public void onInitialSetup(final GameActivityBase activity) {
        final String dataPath = activity.GetGameBaseDirectory();
        final String[] finalNames = new String[]{"data_main"};
        final String[] finalPaths = new String[]{dataPath};
        synchronized (nativeStartupLock) {
            nativeDataSetupDelivered = false;
            nativeActivityCreatedDelivered = false;
        }
        Log.i(TAG, "[!!] onInitialSetup: waiting for bundled data at " + dataPath);
        DataInstaller.prepare(activity);
        new Thread(() -> {
            if (!DataInstaller.awaitReady(30L * 60L * 1000L)) {
                Log.e(TAG, "[!!] data installer timeout: " + DataInstaller.getErrorMessage());
                return;
            }
            queueEvent(new Function0<Unit>() {
                @Override public Unit invoke() {
                    ...
                    synchronized (nativeStartupLock) {
                        nativeDataSetupDelivered = true;
                    }
                    deliverPendingActivityCreated();
                    return Unit.INSTANCE;
                }
            });
        }, "native-data-gate").start();
    }

    private void deliverPendingActivityCreated() {
        final GameActivityBase activity;
        final GameView view;
        final boolean firstInit;
        synchronized (nativeStartupLock) {
            if (!nativeDataSetupDelivered || nativeActivityCreatedDelivered || pendingStartupActivity == null || pendingStartupView == null) {
                return;
            }
            nativeActivityCreatedDelivered = true;
            activity = pendingStartupActivity;
            view = pendingStartupView;
            firstInit = pendingStartupFirstInit;
        }
        Log.i(TAG, "[!!] delivering native activity after data setup");
