...
public abstract class GameActivityBase extends AppCompatActivity {
    ...
    public String GetGameBaseDirectory() {
        try {
            File gameDir = DataInstaller.getTargetDirectory(this);
            if (gameDir == null) return "";
            if (!gameDir.exists() && !gameDir.mkdirs() && !gameDir.exists()) return "";
            return gameDir.getAbsolutePath() + "/";
        } catch (Exception e) {
            Log.e(TAG, "GetGameBaseDirectory failed", e);
            return "";
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (this.baseDirectory == null || this.baseDirectory.length() == 0) {
            throw new IllegalStateException("Diretório privado da data indisponível");
        }
        try {
            GameNative.setNativeStoragePath(this.baseDirectory);
            if ("com.gta.game.SAMP".equals(getClass().getName())) {
                GameNative.bindSampActivity(this);
            }
        } catch (UnsatisfiedLinkError error) {
            Log.e(TAG, "Não foi possível configurar a ponte nativa do SAMP", error);
            throw error;
        }
